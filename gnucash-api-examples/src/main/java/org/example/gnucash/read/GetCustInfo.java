...
public class GetCustInfo {
	...
	private static String custID = "1945133cef2c3fbc372430d09e117e1f";
	// END Example data
	...
	protected void kernel() throws Exception {
		...
		try {
			System.out
					.println(" - via all jobs:  " + cust.getIncomeGeneratedFormatted(GnucashGenerInvoice.ReadVariant.VIA_JOB));
		}
		catch (Exception exc) {
		...
		try {
			System.out
					.println(" - via all jobs: " + cust.getOutstandingValueFormatted(GnucashGenerInvoice.ReadVariant.VIA_JOB));
		}
		catch (Exception exc) {
			System.out.println(" - via all jobs: " + "ERROR");
		}
