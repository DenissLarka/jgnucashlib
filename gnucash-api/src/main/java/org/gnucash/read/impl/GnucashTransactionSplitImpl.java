...
public class GnucashTransactionSplitImpl extends GnucashObjectImpl implements GnucashTransactionSplit {
  ...
  /**
   * @return the lot-id that identifies this transaction to belong to an invoice with that lot-id.
   */
  ...
  public int compareTo(final GnucashTransactionSplit otherSplt) {
    try {
      ...
      if (otherSplt != this) {
        LOGGER.error("Duplicate transaction-split-id!! " + otherSplt.getId() + "[" + otherSplt.getClass().getName()
            + "] and " + getId() + "[" + getClass().getName() + "]\n" + "split0=" + otherSplt.toString() + "\n"
            + "split1=" + toString() + "\n");
