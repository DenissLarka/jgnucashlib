...
public abstract class SimpleAccount implements GnucashAccount {
  ...
  public FixedPointNumber getBalance(final LocalDate date, final Currency currency) {
    ...
    if (!currencyTable.convertFromBaseCurrency(retval, currency.getCurrencyCode())) {
      LOGGER.warn(
          "SimpleAccount.getBalance() - cannot transfer " + "from base-currenty to given currency '" + currency + "'!");
      return null;
    }
   ...
   */
  public FixedPointNumber getBalanceRecursive(final LocalDate date, final String currencyNameSpace,
  ...
  public FixedPointNumber getBalance(final LocalDate date, final String currencyNameSpace, final String currencyName) {
    ...
    // is conversion needed?
    if (getCurrencyNameSpace().equals(currencyNameSpace) && getCurrencyID().equals(currencyName)) {
      return retval;
    }
  ...
  public FixedPointNumber getBalance(final LocalDate date, final Collection<GnucashTransactionSplit> after) {
    ...
    for (Object element : getTransactionSplits()) {
      ...
      boolean bstat;
      try {
        bstat = split.getTransaction().getDatePosted().isAfter(ChronoZonedDateTime.from(date.atStartOfDay()));
      } catch (DateTimeException e) {
        ZonedDateTime zonedDateTime = date.atStartOfDay(ZoneId.systemDefault());
        bstat = split.getTransaction().getDatePosted().isAfter(ChronoZonedDateTime.from(zonedDateTime));
      }
      if (date != null && bstat) {
        // split.getTransaction().getDatePosted().isAfter(
        // ChronoZonedDateTime.from(date.atStartOfDay()) ))
        if (after != null) {
          after.add(split);
        }
        continue;
  ...
  public int compareNamesTo(final Object o) throws ClassCastException {
    ...
    // level accounts
    if (o instanceof GnucashAccount && ((GnucashAccount) o).getParentAccountId() != null && getParentAccountId() != null
