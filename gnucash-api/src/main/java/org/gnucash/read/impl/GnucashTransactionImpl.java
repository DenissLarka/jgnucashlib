...
public class GnucashTransactionImpl extends GnucashObjectImpl implements GnucashTransaction {
  ...
  /**
   * @return the invoices this transaction belongs to (not payments but the transaction belonging to handing out the
   *         invoice)
   */
  public Collection<GnucashGenerInvoice> getInvoices() {
    ...
    for (String invoiceID : invoiceIDs) {
      ...
      if (invoice == null) {
        LOGGER.error(bundle.getMessage("Err_NoInvc4Trans", invoiceID, getId(), getDescription()));
      } else {
        retval.add(invoice);
      }
  ...
  /**
   * sorts primarily on the date the transaction happened and secondarily on the date it was entered.
   *
