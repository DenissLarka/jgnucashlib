...
public class GnucashWritableGenerInvoiceImpl extends GnucashGenerInvoiceImpl implements GnucashWritableGenerInvoice {
	...
	private static GnucashWritableTransaction createPostTransaction(final GnucashWritableFileImpl file,
      ...
      if (invcOwnerType.equals(GCshOwner.TYPE_CUSTOMER)) {
        split1.setAction(Const.ACTION_INVOICE);
      } else if (invcOwnerType.equals(GCshOwner.TYPE_VENDOR)) {
        split1.setAction(Const.ACTION_BILL);
      }
      ...
      if (invcOwnerType.equals(GCshOwner.TYPE_CUSTOMER)) {
        split2.setAction(Const.ACTION_INVOICE);
      } else if (invcOwnerType.equals(GCshOwner.TYPE_VENDOR)) {
        split2.setAction(Const.ACTION_BILL);
      }
	...
	private static GncAccount.ActLots.GncLot createInvcPostLot_Gener(final GnucashWritableFileImpl file,
		...
		// 2.1) add title-slot
		{
			...
			if (ownerType1 == GCshOwner.Type.CUSTOMER) {
				contentStr = Const.ACTION_INVOICE;
			} else if (ownerType1 == GCshOwner.Type.VENDOR) {
				contentStr = Const.ACTION_BILL;
	...
	/**
	 * Called by
	 * ${@link GnucashWritableGenerInvoiceEntryImpl#createCustInvoiceEntry_int(GnucashGenerInvoiceWritingImpl, GnucashAccount, FixedPointNumber,
	 * FixedPointNumber)}.
	 *
	...
	private void updateEntry_taxStuff(final GCshTaxTable taxtable, final FixedPointNumber sumExclTaxes,
		...
		if (!postTransactionTaxUpdated) {
			...
			split.setAction(Const.ACTION_INVOICE);
	...
	private void updateNonTaxableEntry(final FixedPointNumber sumExclTaxes, final FixedPointNumber sumInclTaxes,
		...
		for (GnucashTransactionSplit element : postTransaction.getSplits()) {
			...
			if (split.getAccountID().equals(accountToTransferMoneyFrom)) {
				...
				split.getJwsdpPeer().setSplitAction(Const.ACTION_INVOICE);
