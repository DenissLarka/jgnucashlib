...
public class GnucashWritableFileImpl extends GnucashFileImpl implements GnucashWritableFile {
  ...
  /**
   * Keep the count-data up to date. The count-data is re-calculated on the fly before writing but we like to keep our
   * internal model up-to-date just to be defensive. <gnc:count-data cd:type="commodity">2</gnc:count-data>
   * <gnc:count-data cd:type="account">394</gnc:count-data> <gnc:count-data cd:type="transaction">1576</gnc:count-data>
  ...
  /**
   * keep the count-data up to date. <gnc:count-data cd:type="commodity">2</gnc:count-data>
   * <gnc:count-data cd:type="account">394</gnc:count-data> <gnc:count-data cd:type="transaction">1576</gnc:count-data>
   * <gnc:count-data cd:type="schedxaction">4</gnc:count-data>
   * <gnc:count-data cd:type="gnc:GncCustomer">2</gnc:count-data>
   * <gnc:count-data cd:type="gnc:GncJob">2</gnc:count-data>
   ...
   * Also check the that only valid elements are in the book-element and that they have the correct order.
   * <p>
   * <gnc:count-data cd:type="commodity">2</gnc:count-data> <gnc:count-data cd:type="account">394</gnc:count-data>
  ...
  private void checkAllCountData() {
    ...
    /**
     * <p>
     * Objects of the following type(s) are allowed in the list {@link GncTemplateTransactions} {@link GncGncInvoice}
     * {@link GncGncEntry} {@link GncGncJob} {@link GncGncTaxTable} {@link GncCommodity} {@link GncGncCustomer}
     * {@link GncSchedxaction} {@link GncBudget} {@link GncAccount} {@link GncPricedb} {@link GncTransaction}
     */
  ...
  /**
   * create a GUID for a new element. (guids are globally unique and not tied to a specific kind of entity)
   * 
  ...
  /**
   * Used by GnucashTransactionImpl.createTransaction to add a new Transaction to this file.
   *
  ...
  public Collection<? extends GnucashAccount> getRootAccounts() {
    ...
    if (rootAccounts.size() > 1) {
      ...
      for (GnucashAccount gnucashAccount : rootAccounts) {
        ...
        if (gnucashAccount.getType() != null && gnucashAccount.getType().equals(GnucashAccount.TYPE_ROOT)) {
          root = gnucashAccount;
          continue;
        }
      ...
      LOGGER
          .warn("File has more then one root-account! Attaching excess accounts to root-account: " + roots.toString());
